...
void ModeDrawStar::run()
{
    ...
    { 
        if (wp_nav->reached_wp_destination()) // 已经到达某个端点
        {  
            path_num++;   //目标航点++
            wp_nav->set_wp_destination(path[path_num], false);  // 设置新航点 将下一个航点位置设置为导航控制模块的目标位置
        }
    } 
...
void ModeDrawStar::pos_control_run()  // 注意，此函数直接从mode_guided.cpp中复制过来，不需要该其中的内容
{
    ...
    } else if ((auto_yaw.mode() == AutoYaw::Mode::RATE) || (auto_yaw.mode() == AutoYaw::Mode::ANGLE_RATE)) {

        // roll & pitch from waypoint controller, yaw heading from auto_yaw
